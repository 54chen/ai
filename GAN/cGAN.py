...
def get_generator():
    ...
    generator.add(Dense(256, input_dim=random_dim + 10))
...
def train(epochs=1):
    ...
    for e in range(1, epochs+1):
        ...
        for image_batch in x_train:
            ...
            fake_labels = np.random.choice(np.arange(0, 10, 1), size=(b_size, 1))
            fake_labels = to_categorical(fake_labels, 10)
            noise = tf.concat([noise, fake_labels], axis=1)
            # Generate fake MNIST images
            generated_images = generator.predict(noise,verbose=0)
            real_images = image_batch[:, :-1]
            real_lables = image_batch[:, -1]
            real_lables = to_categorical(real_lables, 10)
            real_lables = tf.reshape(real_lables, [b_size, 10])
...
def plot_generated_images_rad(epoch, generator, examples=100, dim=(10, 10), figsize=(10, 10)):
    noise = np.random.normal(0, 1, size=[examples, random_dim+10])
...
def plot_generated_images(generator, examples=100, dim=(10, 10), figsize=(10, 10)):
    ...
    fake_labels = np.empty((0, 10))
    for label in range(10):
        labels = np.random.choice(np.arange(label, label + 1, 1), size=(10, 1))
...
def plot_generated_images_seeds(generator, examples=50, dim=(10, 5), figsize=(10, 10)):
    ...
    tmp_noise = np.empty((0, 110))

    for label in range(10):
        labels = np.random.choice(np.arange(label, label + 1, 1), size=(5, 1))
        labels = to_categorical(labels, 10)
